...
content = content.replace(
  ...
  const [vid1Size, setVid1Size] = useState(90);
  const [vid2Pos, setVid2Pos] = useState({ x: 50, y: 75 });
  const [vid2Size, setVid2Size] = useState(90);`
);
...
// Clean duplicate startPosX from above replacement manually just in case:
content = content.replace(
...
content = content.replace(
  ...
  `{frameMode === 'frame1' && (
                    <img
                      src="/images/rell.png"` // To prevent double replacin' if it somehow already ran
); 
if (!content.includes('frameMode === \'frame2\' && (\n                    <img\n                      src="/images/frame.jpg"')) {
content = content.replace(
  `{frameMode === 'frame1' && (
                    <img
                      src="/images/rell.png"
...
// Add video2
if (!content.includes('frameMode === \'frame2\' && video2Src')) {
content = content.replace(
  `{frameMode === 'frame2' && (
                    <img
                      src="/images/frame.jpg"
...
// Tab insertion
if (!content.includes('tab === \'layout\'')) {
content = content.replace(
...
// Layout Tab Content
if (!content.includes('activeTab === \'layout\' && (')) {
content = content.replace(
  `{activeTab === 'trim' && (`,
  `{activeTab === 'layout' && (
